...
static DOWNLOADED: Mutex<Option<UpdateInfo>> = Mutex::new(None);
...
fn get_update_manager(
    ...
    let source = sources::GithubSource::new(&config.url, None, false);
...
pub fn check_and_download_updates_with_progress(
    ...
    if let UpdateCheck::UpdateAvailable(updates) = um.check_for_updates()? {
        ...
        let (sx, rx) = mpsc::channel::<i16>();

        let forwarder = std::thread::spawn(move || {
            while let Ok(progress) = rx.recv() {
                let _ = progress_sink.add(progress.min(99));
            }
            progress_sink
        });
        let result = um.download_updates(&updates, Some(sx));
        let progress_sink = forwarder.join().map_err(|_| anyhow::anyhow!("Progress worker failed"))?;
        result?;
        *DOWNLOADED.lock().map_err(|_| anyhow::anyhow!("Update lock poisoned"))? = Some((*updates).clone());
        let _ = progress_sink.add(100);
...
pub fn update_and_restart(channel: Option<String>, allow_downgrade: Option<bool>) -> Result<()> {
    let downloaded = DOWNLOADED.lock().map_err(|_| anyhow::anyhow!("Update lock poisoned"))?;
    let updates = downloaded.as_ref().ok_or_else(|| anyhow::anyhow!("Download an update before restarting"))?;
    let um = get_update_manager(channel, allow_downgrade)?;
